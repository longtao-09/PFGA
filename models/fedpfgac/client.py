...
class Client(ClientModule):
    ...
    def update(self, update):
        self.cluste_id = update['cluster_labels'][self.client_id]
        # set_state_dict(self.model, update[f'model_{self.cluste_id}'], self.gpu_id, skip_stat=True, Local=True)
    ...
    def train_rep(self, model, batch, num_classes, alpha=0.5, beta=3.0, gamma=2.0, train_edge=None, new_label=None):
        ...
        if self.sd['global'] is not None:
            ...
            for c_id, model_a in self.sd['global'].items():
                if c_id == 'cluster_labels':
                    break
                cluster_id = int(c_id.split("_")[-1])
                if cluster_id != self.cluste_id + 1:
            ...
            set_state_dict(self.model, self.sd['global'][f'model_{self.cluste_id}'], self.gpu_id, skip_stat=True,
                           Local=True)
        if train_edge is not None:
            adj = train_edge.train_pos_edge_index
