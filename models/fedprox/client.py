...
class Client(ClientModule):
    ...
    def train(self):
        for ep in range(self.args.n_eps):
            ...
            reg_loss = torch.tensor(0.0, requires_grad=True).cuda(self.gpu_id)
            for param, global_param in zip(self.model.parameters(), self.sd['global_models'].parameters()):
                reg_loss += torch.norm(param - global_param) ** 2
            train_lss = train_lss + 0.1 * reg_loss
            train_lss.backward()
            self.optimizer.step()
